...
- 区块 ------->区块分组关联变化: 
由nodejs的blocks_groups__block_groups_blocks重构后直接是在区块t_block表里添加了block_group_id字段进行关联，就不需要nodejs的blocks_groups__block_groups_blocks表了
...
- 数据库表关系更清晰明了，删除了一些没用的表关系，比如区块和区块分组的表关系，是直接在区块表里加分组的id做为外键
