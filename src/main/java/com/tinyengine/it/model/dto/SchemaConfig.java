...
public class SchemaConfig {
    ...
    public SchemaConfig() {
        ...
        this.appConvert = Stream.of(new Object[][]{
                {"id", "appId"},
                {"createdBy", "creator"},
        ...
        this.appInclude = Arrays.asList(
                "id", "name", "tenant", "git_group",
                "project_name", "is_demo", "description",
                "createdBy", "created_at", "updated_at",
                "branch", "global_state"
        );

        this.appFormat = Stream.of(new Object[][]{
                {"id", "toFormatString"},
        ...
        this.pageMetaConvert = Stream.of(new Object[][]{
        ...
        this.pageMetaInclude = Arrays.asList(
                "id", "title", "page_desc", "createdBy",
                "parentId", "created_at", "updated_at",
                "isHome", "isBody", "group", "route",
                "occupier"
        );

        this.pageMetaFormat = Stream.of(new Object[][]{
                {"created_at", "toLocalTimestamp"},
                {"updated_at", "toLocalTimestamp"},
                {"isBody", "toRootElement"},
                {"group", "toGroupName"},
                {"createdBy", "toCreatorName"}
        }).collect(Collectors.toMap(data -> (String) data[0], data -> (String) data[1]));

        // Initialize PageContentConfig
        this.pageContentInclude = Arrays.asList(
                "fileName", "componentName", "props", "css", "children", "methods", "state", "lifeCycles"
        );

        // Initialize FolderConfig
        this.folderConvert = Stream.of(new Object[][]{
                {"name", "folderName"},
                {"route", "router"},
                {"created_at", "gmt_create"},
                {"updated_at", "gmt_modified"}
        ...
        this.folderInclude = Arrays.asList(
                "name", "route", "created_at", "updated_at", "id", "parentId", "depth"
        );

        this.folderFormat = Stream.of(new Object[][]{
                {"created_at", "toLocalTimestamp"},
                {"updated_at", "toLocalTimestamp"}
        }).collect(Collectors.toMap(data -> (String) data[0], data -> (String) data[1]));
    }
