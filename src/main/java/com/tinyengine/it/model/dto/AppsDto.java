...
public class AppsDto implements Serializable {
    ...
    @Schema(name = "block_categories", description = "app应用下关联的区块分类")
    List<BlockCategories> block_categories;

    @TableField(exist = false)
    @Schema(name = "block_groups", description = "app应用下关联的区块分组")
    List<BlockGroupsDto> block_groups;
}
